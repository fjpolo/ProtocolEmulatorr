...
set ASM_FILE=%~dp0..\examples\echo_configurable.asm
...
echo.
python "%~dp0..\python\omnibus_loader.py" --set-baud %~1 --file "%ASM_FILE%" --terminal
exit /b %ERRORLEVEL%

:default_baud
echo [*] No baud rate specified — using default 115200 baud
python "%~dp0..\python\omnibus_loader.py" --file "%ASM_FILE%" --terminal
exit /b %ERRORLEVEL%
