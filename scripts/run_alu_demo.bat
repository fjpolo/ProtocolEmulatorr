...
:do_interactive
set ASM_FILE=%~dp0..\examples\alu_interactive.asm
...
:do_selftest
set ASM_FILE=%~dp0..\examples\alu_selftest.asm
...
:do_packet
set ASM_FILE=%~dp0..\examples\packet_parser_demo.asm
echo [*] Mode: Autonomous Packet Parser ^& Validator
echo [*] Microcode: examples\packet_parser_demo.asm
echo [*] Assembling and uploading to FPGA IMEM over UART...
python "%~dp0..\python\omnibus_loader.py" --file "%ASM_FILE%" %EXTRA_ARGS%
exit /b %ERRORLEVEL%
